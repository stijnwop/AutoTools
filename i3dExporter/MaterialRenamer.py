                ...
                if ( m_node ):
                    if ( 'file' == cmds.nodeType( m_node ) ):
                        try:
                            ...
                            m_file = os.path.splitext( m_file )[0]
                            m_file = os.path.split( m_file )[1]
                            m_strList = [ "_diffuse", "_normal", "_specular" ]
                    ...
                    try:
                        ...
                        for m_str in m_strList:
                            ...
                            if ( -1 != m_strIndex ):
                                m_file = m_file[ :m_strIndex ]
                        try:
                            cmds.rename( m_mat, "%s_mat" %m_file)
                            m_result = True
                        except:
...
def setNormalsToRAWcolorSpace( ):
    m_fileList = cmds.ls( type = 'file' )
    for m_fileNode in m_fileList:
        if ( not isObjFromReferencedFile( m_fileNode ) ):
            try:
                m_file = cmds.getAttr( "{}.fileTextureName".format(m_fileNode) )
                m_colorSpace = cmds.getAttr( "{}.colorSpace".format(m_fileNode) )
            except:
                print( "Can't getAttr {0}.fileTextureName or {0}.colorSpace".format(m_fileNode) )
            else:
                # get
                m_file = os.path.splitext( m_file )[0]
                m_file = os.path.split( m_file )[1] 
                # find normal
                m_strIndex = m_file.lower().find( "_normal" )
                if ( -1 != m_strIndex ):
                    try:
                        cmds.setAttr( "{}.colorSpace".format(m_fileNode), 'Raw', type = "string" )
                    except:
                        print( "Can't set attr {}.colorSpace".format(m_fileNode) )
                    else:
                        print( "setAttr {}.colorSpace to Raw Successfully!".format(m_fileNode) ) 
...
def getFirstFileNode( m_nodeInput, m_result = None ):
    ...
    if ( m_connectedNodes ):
        for m_node in m_connectedNodes:
            if    ( 'file' == cmds.nodeType( m_node ) ):
                m_file      = cmds.getAttr( "%s.fileTextureName" %m_node )
                m_fileName  = os.path.splitext( m_file )[0]
                if ("default" in m_fileName):
                    pass
                else:
                    m_result = m_node
                    return m_result                
            elif  ( 'bump2d' == cmds.nodeType( m_node ) ):
                m_result = getFirstFileNode( m_node, m_result )
    return m_result 
